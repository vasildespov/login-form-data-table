...
- For the connectivity notification feature I implemented a hook that listens to the online and offline window events. In order for the image in the modal to load when the user goes offline I had to implement a service worker that puts the image from /public in the cache storage. Otherwise Vite makes a request to /public but it fails because it still requires internet connection.
